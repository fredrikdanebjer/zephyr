...
static int set_available_contexts(uint16_t contexts, uint16_t *available,
				  uint16_t supported)
{
	LOG_DBG("set_available_contexts");

	if (contexts & ~supported) {
		LOG_DBG("set_available_contexts bye 1");
		return -ENOTSUP;
	}

	if (contexts == *available) {
		LOG_DBG("set_available_contexts bye 2");
		return 0;
	}

	*available = contexts;

	LOG_DBG("set_available_contexts defer work");
	bt_conn_foreach(BT_CONN_TYPE_LE, defer_value_ntf,
			UINT_TO_POINTER(FLAG_AVAILABLE_AUDIO_CONTEXT_CHANGED));

	LOG_DBG("set_available_contexts bye 3");
...
{
	LOG_DBG("set_snk_available_contexts");
...
static void defer_value_ntf(struct bt_conn *conn, void *data)
{
	...
	LOG_DBG("For %p setting bit %u", client, POINTER_TO_UINT(data));
...
#if defined(CONFIG_BT_PAC_SNK_NOTIFIABLE)
	bt_conn_foreach(BT_CONN_TYPE_LE, defer_value_ntf,
			UINT_TO_POINTER(FLAG_SINK_PAC_CHANGED));

#endif /* CONFIG_BT_PAC_SNK_NOTIFIABLE */

#if defined(CONFIG_BT_PAC_SRC_NOTIFIABLE)
	bt_conn_foreach(BT_CONN_TYPE_LE, defer_value_ntf,
			UINT_TO_POINTER(FLAG_SOURCE_PAC_CHANGED));
